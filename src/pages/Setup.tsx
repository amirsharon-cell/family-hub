    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center p-6">
      <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm">
        <div className="text-center mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-900">Set up calendars</h1>
          <p className="text-sm text-gray-500 mt-1">Hi {user?.name?.split(' ')[0]}! Let's connect your family calendars.</p>
        </div>
        ...
        {step === 'scan' && (
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              Family Hub uses two Google Calendars:
            </p>
            <ul className="text-sm text-gray-600 space-y-1 ml-4 list-disc">
              <li><strong>Family Hub</strong> – shared events (dinners, shows, trips…)</li>
              <li><strong>Family Car</strong> – car bookings</li>
            </ul>
            <button
              onClick={scan}
              disabled={loading}
            ...
            >
              {loading ? 'Scanning…' : 'Scan my Google Calendars'}
            </button>
            <button
              onClick={() => setStep('manual')}
              className="w-full text-sm text-gray-500 hover:text-gray-700 py-2"
            >
              Enter calendar IDs manually
            </button>
          </div>
        )}
        ...
        {step === 'found' && found && (
          <div className="space-y-4">
            <div className="bg-green-50 rounded-xl p-4 space-y-2">
              <p className="text-green-700 font-medium text-sm">✅ Found your family calendars!</p>
        ...
        {step === 'create' && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              No existing Family Hub calendars found. Create them now — then share them with your family from Google Calendar.
            </p>
            <button
            ...
            >
              {loading ? 'Creating calendars…' : 'Create Family Calendars'}
            </button>
            <button
              onClick={() => setStep('manual')}
              className="w-full text-sm text-gray-500 hover:text-gray-700 py-2"
            >
              I already have calendar IDs
            </button>
          </div>
        )}
        ...
        {step === 'manual' && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Family Hub calendar ID</label>
              <input
                type="text"
                value={manualEvents}
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Family Car calendar ID</label>
              <input
                type="text"
                value={manualCar}
